...
bool AV1Encoder::initCodec(const EncoderConfig& config)
{
    ...
    if (config.hwAccel == HardwareAccel::NVENC) {
        ...
        if (codec) { spdlog::info("AV1Encoder: Using NVENC AV1"); m_hwAccel = true; }
    }
    if (!codec) { codec = avcodec_find_encoder_by_name("libsvtav1"); m_hwAccel = false; }
    if (!codec) { codec = avcodec_find_encoder(AV_CODEC_ID_AV1); }
    if (!codec) { m_lastError = "No AV1 encoder found"; spdlog::error("{}", m_lastError); return false; }
    ...
    if (!m_hwAccel) {
        ...
        int svtPreset = 8; // Default: fast balance
    ...
    if (avcodec_open2(m_codecCtx, codec, nullptr) < 0) {
        m_lastError = "AV1Encoder: Failed to open codec";
